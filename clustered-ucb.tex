...
Our study concludes that theoretically the regret of ClusUCB is lower than UCB1, MOSS and UCB-Revisited while empirically we compared against UCB2, KL-UCB, UCB-Tuned, DMED and Median Elimination under certain cases when the $\Delta_{i}$'s are small and $K$ is large. Such cases can frequently occur in web advertising scenarios where the $r_{i}$ are small and a large number of ads are available in the pool. For the critical case when $\Delta_{i:r_{i}<r*}, \forall i\in A$ are equal then ClusUCB performs better than UCB-Revisited, UCB1, UCB-Tuned, KL-UCB, MOSS and EXP3 with sufficient tuning of parameters as shown empirically. Also ClusUCB scales well with large $K$ as compared with UCB1, EXP3, MOSS, KL-UCB and Thompson Sampling since it is eliminating sub-optimal arms after every round. 
