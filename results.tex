...
& + \dfrac{12\sqrt{\log (KT)}}{\Delta_{i}}  + \dfrac{96\log{(\frac{T\Delta_{i}^{2}}{\sqrt{\log (KT)}})}}{\Delta_{i}}\bigg\rbrace \\
...
&+ \sum\limits_{\substack{i\in A,\\0\leq\Delta_{i}\leq b}}\dfrac{8\sqrt{\log (KT)}}{\Delta_{i}} + \max\limits_{i:\Delta_{i}\leq b}\Delta_{i}T
\end{align*} 
...
\begin{corollary}[\textbf{\textit{Gap-independent bound}}]
\label{Result:Corollary:2}
Considering the same gap of $\Delta_{i} = \Delta =\sqrt{\dfrac{K\log K}{T}}$ for all $a_{i:i\neq *}$ and with $\psi=K^{2}T$, $\rho_{a}=\frac{1}{4}$ and $\rho_{s}=\frac{1}{2}$, 
 we have the following gap-independent bound for the regret of ClusUCB:
\begin{align*}
\E [R_T] &\!\le\!  6\dfrac{\sqrt{KT}}{p} + 128\sqrt{KT\log K} + \dfrac{64\sqrt{KT}\log{(\log K)}}{\sqrt{\log K}}\\
 &+ 16\sqrt{\dfrac{T}{K\log K}} + 8\sqrt{\dfrac{T}{e}}
\end{align*}
\end{corollary}
\begin{proof}
 See Appendix \ref{App:Proof:Corollary:2}.
