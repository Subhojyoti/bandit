...
\begin{remark}
By setting $b=\dfrac{\sqrt{K\log K}}{T}$, the term $max_{i:\Delta\leq b}\Delta_{i}T$ gets trivially bounded by $\sqrt{K\log K}$ as mentioned in UCB-Improved which makes the logarithmic terms the main term for suitable $b$. 
\end{remark}
...
\begin{remark}
So, the most significant part of the regret bound is $\sum_{i\in A:\Delta\geq b}\dfrac{32(K+p)\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{p\Delta_{i}}$. For $b\approx \dfrac{\sqrt{K\log K}}{T}$ in the term we get, $K\dfrac{(K+p)\rho_{s}\log{(\psi_{m}T\dfrac{K^{2}(\log K)^{2}}{\rho_{s}^{2}T^{2}})}}{p \dfrac{\sqrt{K\log K}}{T}}=\dfrac{KT(K+p)\rho_{s}\log{(\psi_{m}\dfrac{K^{2}(\log K)^{2}}{\rho_{s}^{2}T^{3}})}}{p \sqrt{K\log K}}$. Now for $p= \dfrac{K}{2}$(considering each cluster gets atleast $2$ arms) we get $\dfrac{(K+p)}{p}=  2$. Again, since there are $\big \lfloor \dfrac{1}{2}\log_{2} \dfrac{T}{e}\big\rfloor$ rounds then $\rho_{s}= \sqrt{\dfrac{e}{T}}$(from Definition $1$, \textbf{Appendix E}) and taking $\psi_{m}=K^{2}T$(from Definition $3$, \textbf{Appendix D}). So we get, $\dfrac{\sqrt{KT}\log{(\dfrac{K^{4}(\log K)^{2}}{eT})}}{\sqrt{\log K}} \leq \dfrac{\sqrt{KT}\log{(\dfrac{K^{4}(\log K)^{2}}{T})}}{\sqrt{\log K}}$.
\end{remark}
...
\begin{remark}
Again, of the several terms in the regret bound, the terms like  $\sum_{i\in A:\Delta\geq b}\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+\frac{3}{2}}}{\Delta_{i}^{4\rho_{s}-1}} \bigg)$ can become very large given a small $\Delta_{i}$ and large $T$. Drawing from the conclusion of our exploration regulatory factor in \textbf{Appendix D}, we can introduce $\psi_{m}=K^{2}T$ in our confidence interval thereby when $\rho_{s}=\sqrt{\dfrac{e}{T}}$ then the term essentially becomes $\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+\frac{3}{2}}}{(K^{2}T\Delta_{i})^{4\rho_{s}-1}} \bigg) \approx KT\Delta_{i} = KT\dfrac{\sqrt{K\log K}}{T}=K\sqrt{K\log K}$. 
\end{remark}

\begin{remark}
So, we see that putting $b=\dfrac{\sqrt{K\log K}}{T}$, the corresponding terms of $\rho_{s}$ and $\psi_{m}$ and then combining remark 3 and 4 we get the worst case gap-independent regret bound of $\dfrac{\sqrt{KT}\log{(\dfrac{K^{4}(\log K)^{2}}{T})}}{\sqrt{\log K}}$ which coincides with the term $bT$ of the regret bound in Theorem 1, except the term  $\dfrac{\log{(\dfrac{K^{4}(\log K)^{2}}{T})}}{\sqrt{\log K}}$. When we consider that $T\approx K^{4}(\log K)^{2}$, this term becomes negligible and so the regret bound becomes better than UCB-Improved non-gap based regret upper bound of $\sqrt{KT}\dfrac{\log(K\log K)}{\sqrt{\log K}}$ and slightly better than the MOSS bound of $\sqrt{KT}$.
\end{remark}
