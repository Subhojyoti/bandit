...
\E [R_{T}] \le & 74\dfrac{\sqrt{T}\log K}{\sqrt{K}} \!+\! \dfrac{32\sqrt{T\log K}\log{(\log K)}}{\sqrt{K}}\\
 &\!+\! 4\sqrt{KT} + 128\sqrt{KT\log K}\\
 &\!+\! \dfrac{64\sqrt{KT}\log{(\log K)}}{\sqrt{\log K}} + \dfrac{5.3 \sqrt{T}\log K^{\frac{3}{2}} }{\sqrt{K}}\\
 & \!+\!  \dfrac{10.6K}{K+\log K}\sqrt{KT\log K} + \dfrac{10.6 K}{K+\log K}\sqrt{KT}\\ 
\end{align*}
\end{corollary}
\begin{proof}
...
Thus, we observe that clustering in conjunction with improved exploration via $\rho_{a},\rho_{s}$,$p$ and $\psi$ helps in reducing the factor associated with $\sqrt{KT\log K}$ for the gap-independent error regret bound for ClusUCB. A table containing the regret error bound is shown in Appendix \ref{App:E}. 
