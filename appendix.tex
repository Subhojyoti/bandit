...
\subsubsection*{Case b1: \textit{If an optimal arm $a^{*}\in B_{m_{i}}$ then bound the maximum pull of all arms $a_{i}\in A^{'}$} till $ m_{i}$ }
...
&\leq\sum_{m_{*}=0}^{max_{j\in A^{'}}m_{j}}\sum_{i\in A^{''}:m_{i}>m_{*}}\bigg(\dfrac{2}{(\psi  T\epsilon_{m}^{2})^{\rho_{a}}} \bigg).T.2\sqrt{\rho_{a}\epsilon_{m}}\\
