...
%\subsubsection{Case a21:} For some sub-optimal arm $a_{s_{k}}\in C_{g}$, $\hat{r}_{a_{s_{k}}}+c_{g_{s_{k}}}< \hat{r}_{a_{max_{s^{*}}}}-c_{g_{s_{k}}}$. But, this probability has already been bounded by the case $a1$ since $r_{max_{s^{*}}} < r^{*}$ and so it can be no worse than $\bigg(\dfrac{2}{(T\epsilon_{g_{s_{k}}}^{2})^{\rho_{s}}}\bigg)$. After summing over all arms in $A$ and bounding trivially by $T\Delta_{i}$ we get the same result as above. The regret can be no more than,
\newline
$\sum_{i\in A}\bigg(\dfrac{2^{1+4\rho_{s}}\rho_{s}^{2\rho_{s}}T^{1-\rho_{s}}}{\Delta_{i}^{4\rho_{s}-1}}\bigg)$
%\subsubsection{Case a22:} Again, for some sub-optimal arm $a_{s_{k}}\in C_{g}$, $\hat{r}_{a_{s_{k}}}+c_{g_{s_{k}}} >= \hat{r}_{a_{max_{s^{*}}}}-c_{g_{s_{k}}}$. So, the cluster $s_{k}$ survives and all the $\bigg\lceil \dfrac{K}{p} \bigg\rceil$ arms in $s_{k}$ gets pulled $n_{g_{s_{k}}}$ number of times. This case is exactly like Case $b1$ which we handle next.
...
\begin{remark}
\label{App:E:Rem:1}
Putting the value of $\psi_{m}=(K)^{\frac{1}{\rho_{s}}}$, using \textbf{Appendix E, Definition 1} for $\rho_{s}$ and $b=\sqrt{\dfrac{K\log K}{T}}$ in $\sum_{i\in A:\Delta_{i}\geq b}\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+3}}{\psi_{m}^{\rho_{s}}\Delta_{i}^{4\rho_{s}-1}} \bigg)=K\bigg(\dfrac{T^{1-\sqrt{\frac{e}{T}}}(\sqrt{\frac{e}{T}}^{2\sqrt{\frac{e}{T}}})2^{2\sqrt{\frac{e}{T}}+3}}{{(K)^{\sqrt{\frac{T}{e}}*\sqrt{\frac{e}{T}}}}{(\Delta_{i})^{4*\sqrt{\frac{e}{T}}-1}}} \bigg) \approx \dfrac{8KT\Delta_{i}}{K}=\dfrac{8T\sqrt{K\log K}}{\sqrt{T}}=8\sqrt{KT\log K}$. A similar result can also be shown for $\sum_{i\in A:0\leq\Delta_{i}\leq b}\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+3}}{b^{4\rho_{s} -1}} \bigg)\approx \dfrac{8KT\Delta_{i}}{K} = \sqrt{eTK}$ for $b=\sqrt{\dfrac{e}{T}}$. 

\par For the term $\sum_{i\in A:\Delta_{i}\geq b}\dfrac{32\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}}$ by setting $b\approx \sqrt{\dfrac{K\log K}{T}}$ and the above parameter values for $\psi_{m},\rho_{s}$ in the term we get, 
$\dfrac{32K\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}}\leq \dfrac{32K\rho_{s}\log{(\psi_{m})}}{\Delta_{i}}+ \dfrac{32K\rho_{s}\log(T\dfrac{\Delta_{i}^{4}}{\rho_{s}^{2}})}{\Delta_{i}}$
...
\newline
So, the total bound comes of as $\max\bigg\lbrace32\sqrt{KT} + \dfrac{32\sqrt{eK}\log(K^{2}(\log K)^{2})}{\sqrt{\log K}}, 8\sqrt{KT\log K}\bigg\rbrace$
\end{remark}
