...
\begin{tabular}{p{1cm}p{3cm}p{2cm}p{2cm}p{8cm}}
...
\hline \\
3	&$\psi_{m}=K^{2}T$         & $K^{2} T$  &$K^{2} T$ & Constant factor. Used in \textbf{Remark \ref{Result:Rem:3}}. \\ 
\hline \\ 
4	&$\psi_{m}=\log{T}$         & $\log{T}$  &$\log{T}$ & Constant factor. Used in experiment. \\  
\hline \\
5	&$\psi_{m}=K^{\frac{1}{\rho_{s}}}$         & $K^{\sqrt{\frac{T}{e}}}$  &$K$ & Used in \textbf{Remark \ref{App:E:Rem:1}}. $\rho_{s}$ from \textbf{Definition 1, Appendix E}. \\  
\end{tabular}
\end{center}	
\end{table}
...
\begin{center}
\begin{tabular}{p{1cm}p{5cm}p{1cm}p{1cm}p{5cm}}
...
\hline\\ 
2	&$\rho_{s},\rho_{a}=max\bigg\lbrace\dfrac{1}{2^{m}},\dfrac{1}{4}\bigg\rbrace$         & $1,1$  &$\dfrac{1}{4},\dfrac{1}{4}$ & A less risky definition having a higher lower bound than Definition $1$ since $\sqrt{\dfrac{e}{T}}< \dfrac{1}{4}$ if $T> 16e$\\
\hline\\ 
...
\hline\\
5	&$\rho_{s}=max\bigg\lbrace\dfrac{1}{2^{m}},\dfrac{1}{4}\bigg\rbrace$\newline $\rho_{a}=\dfrac{1}{2^{4m+1}}$   & $\dfrac{1}{2},\dfrac{1}{2}$  &$\dfrac{e}{2T},\dfrac{e^{2}}{2T^{2}}$ & Used in Proof.
\end{tabular}
\end{center}
\end{table}
...
\begin{remark}
\label{App:E:Rem:1}
Putting the value of $\psi_{m}=(K)^{\frac{1}{\rho_{s}}}$(\textbf{Appendix D, Definition 5}) and using \textbf{Appendix E, Definition 1} for $\rho_{s}$ and $b=\sqrt{\dfrac{K\log K}{T}}$ in $\sum_{i\in A:\Delta_{i}\geq b}\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+3}}{\psi_{m}^{\rho_{s}}\Delta_{i}^{4\rho_{s}-1}} \bigg)=K\bigg(\dfrac{T^{1-\sqrt{\frac{e}{T}}}(\sqrt{\frac{e}{T}}^{2\sqrt{\frac{e}{T}}})2^{2\sqrt{\frac{e}{T}}+3}}{{(K)^{\sqrt{\frac{T}{e}}*\sqrt{\frac{e}{T}}}}{(\Delta_{i})^{4*\sqrt{\frac{e}{T}}-1}}} \bigg) \approx \dfrac{8KT\Delta_{i}}{K}=\dfrac{8T\sqrt{K\log K}}{\sqrt{T}}=8\sqrt{KT\log K}$. A similar result can also be shown for $\sum_{i\in A:0\leq\Delta_{i}\leq b}\bigg(\dfrac{T^{1-\rho_{s}}\rho_{s}^{2\rho_{s}}2^{2\rho_{s}+3}}{b^{4\rho_{s} -1}} \bigg)\approx \dfrac{8KT\Delta_{i}}{K} = \sqrt{eTK}$ for $b=\sqrt{\dfrac{e}{T}}$. 
...
$\dfrac{32K\rho_{a}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{a}^{2}})}}{\Delta_{i}}\leq \dfrac{32K\rho_{a}\log{(\psi_{m})}}{\Delta_{i}}+ \dfrac{32K\rho_{a}\log(T\dfrac{\Delta_{i}^{4}}{\rho_{a}^{2}})}{\Delta_{i}}$
...
\par For the term $\sum_{i\in A:\Delta_{i}\geq b}\dfrac{32\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}}$ by setting $b\approx \sqrt{\dfrac{K\log K}{T}}$ and the above parameter values for $\psi_{m},\rho_{s}=\max{\bigg\lbrace\dfrac{1}{4},\dfrac{1}{2^{m}}\bigg\rbrace}$ in the term we get,
$\leq \dfrac{32K\rho_{s}\log{(\psi_{m})}}{\Delta_{i}}+ \dfrac{32K\rho_{s}\log(T\dfrac{\Delta_{i}^{4}}{\rho_{s}^{2}})}{\Delta_{i}}$
$=\dfrac{32K\dfrac{1}{4}\log{(K)^{4}}}{\sqrt{\dfrac{K\log K}{T}}} + \dfrac{32K\sqrt{\dfrac{1}{4}}\log(T\dfrac{K^{2}(\log K)^{2}}{T^{2}})}{\sqrt{\dfrac{K\log K}{T}}}$
$=32\sqrt{KT\log K} + \dfrac{8\sqrt{KT}\log(\dfrac{4*K^{2}(\log K)^{2}}{T})}{\sqrt{\log K}}$
\newline
So, the total bound comes of as $\bigg\lbrace 32\sqrt{KT} + 40\sqrt{KT\log K} + \dfrac{8\sqrt{KT}\log(\dfrac{4*K^{2}(\log K)^{2}}{T})}{\sqrt{\log K}} \bigg\rbrace$, ignoring the other small terms.
\end{remark}

\begin{remark}
\label{App:E:Rem:2}
Another important distinction we want to make is the apparent use of clustering. The regret bound and error bound when a sub-optimal arm or a sub-optimal cluster eliminates $a^{*}$ or $s^{*}$ is given below.
\newline
\begin{table}
\caption{Regret Bound on Arm Elimination and Pulls}
\label{App:E:table:1}
\begin{center}
\begin{tabular}{p{0.5cm}p{11cm}p{6cm}}
\multicolumn{1}{c}{\bf Bound} &\multicolumn{1}{c}{\bf Regret Bound on Arm Elimination and Pulls} &\multicolumn{1}{c}{\bf Remarks} \\
\hline \\
1)	&$\sum_{i\in A:\Delta_{i}\geq b}\bigg\lbrace\bigg(\dfrac{2^{1+4\rho_{a}}\rho_{a}^{2\rho_{a}}T^{1-\rho_{a}}}{(\psi_{m})^{\rho_{a}}\Delta_{i}^{4\rho_{a}-1}}\bigg) + \bigg(\Delta_{i}+\dfrac{32\rho_{a}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{a}^{2}})}}{\Delta_{i}}\bigg)\bigg\rbrace$ & Only Arm Elimination with $\rho_{a}$ from \textbf{Definition 1,Appendix E} gives $ 8\sqrt{KT\log K} + 32\sqrt{KT} $\\
%+ \dfrac{32\sqrt{eK}\log(K^{2}(\log K)^{2})}{\sqrt{\log K}}$
\hline\\
2)	&$ \sum_{i\in A:\Delta_{i}\geq b}\bigg\lbrace\underbrace{\bigg(\dfrac{2^{1+4\rho_{s}}\rho_{s}^{2\rho_{s}}T^{1-\rho_{s}}}{(\psi_{m})^{\rho_{s}}\Delta_{i}^{4\rho_{s}-1}}\bigg)}_{\text{case a1}} + \underbrace{\bigg(\dfrac{2^{1+4\rho_{s}}\rho_{s}^{2\rho_{s}}T^{1-\rho_{s}}}{(\psi_{m})^{\rho_{s}}\Delta_{i}^{4\rho_{s}-1}}\bigg)}_{\text{case a2}} $ \newline $ + \underbrace{\bigg(\Delta_{i}+\dfrac{32\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}}\bigg)}_{\text{case b1}}\bigg\rbrace$ & Only Cluster Elimination with $\rho_{a}=\rho_{s}$ from \textbf{Definition 1,Appendix E} gives $ 16\sqrt{KT\log K} + 32\sqrt{KT} $. Hence, this is larger than bound $1$.\\
%+ \dfrac{32\sqrt{eK}\log(K^{2}(\log K)^{2})}{\sqrt{\log K}}$
\hline\\
3)	&$\sum_{i\in A:\Delta_{i}\geq b} \bigg\lbrace \underbrace{\bigg(\dfrac{2^{1+4\rho_{a}}\rho_{a}^{2\rho_{a}}T^{1-\rho_{a}}}{(\psi_{m})^{\rho_{a}}\Delta_{i}^{4\rho_{a}-1}}\bigg)}_{\text{case a, Arm Elim}} + \underbrace{\bigg(\dfrac{2^{2+4\rho_{s}}\rho_{s}^{2\rho_{s}}T^{1-\rho_{s}}}{(\psi_{m})^{\rho_{s}}\Delta_{i}^{4\rho_{s}-1}}\bigg)}_{\text{case a, Clus Elim}} $ \newline $+ \underbrace{\bigg(\Delta_{i}+\dfrac{32\rho_{a}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{a}^{2}})}}{\Delta_{i}}\bigg)}_{\text{case b1, Arm Elim}} +\underbrace{\bigg(\Delta_{i}+\dfrac{32\rho_{s}\log{(\psi_{m}T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}}\bigg)}_{\text{case b1, Clus Elim}}\bigg\rbrace$ & Arm \& Cluster Elimination with $\rho_{a}<<\rho_{s}$ from \textbf{Definition 5,Appendix E} gives $40\sqrt{KT\log K} + 43.2\sqrt{KT} +\dfrac{8\sqrt{KT}\log{(\dfrac{K^{2}(\log K)^{2}}{T})}}{\sqrt{\log K}}$. This can become better than bound $1$ and $2$ with $T >> K^{2}(\log K)^{2}$.
\end{tabular}
\end{center}
\end{table}
