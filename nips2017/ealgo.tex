...
\For{$t=K+1,..,T$}	
\State Pull arm $i\in \argmax_{j\in B_{m}}\bigg\lbrace \hat{r}_{j} + \sqrt{\frac{\log{(\psi T\epsilon_{m}^{2})}}{ z_{j}}} \bigg\rbrace$, where $z_j$ is the number of times arm $j$ has been pulled
%\State $t:=t+1$
\ArmElim
