...
Thus,   the probability that a sub-optimal arm $a_{i}$ is not eliminated in any round $m_{i}$ is bounded above by  $\bigg(\frac{2}{(\psi T\epsilon_{m_{i}}^{2})^{\rho_{a}}}\bigg)$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
\textbf{Case a2:} \textit{In round $m_i$, $a_{i} \in s_{k}$ for some $s_k \ne s^*$.} % where $r_{\max_{s_{k}}}\leq r^{*}$ 
...
We can show that the probability of $a_{i}$ not getting eliminated cannot be worse than Case $a1$ given that $\sqrt{\rho_{a}\epsilon_{m}}< \frac{\Delta^{'}_{i}}{2}$ where $\Delta^{'}_{i}=r_{\max_{s_{k}}} - r_{i}\geq\Delta_{a_{\max_{s_{k}}}}$ such that $r_{i}\in s_{k}$. Plugging in this $\Delta^{'}_{i}$ in Case $a1$ and bounding the complementary events like \ref{eq:armelim-casea} with $r_{i}$ and $r_{a_{\max_{s_{k}}}}$, we can derive a similar bound where $\Delta^{'}_{i}\geq \Delta_{a_{\max_{s_{k}}}}$ because otherwise $\sqrt{\rho_{s}\epsilon_{g_{s_{k}}}}< \frac{\Delta_{a_{\max_{s_{k}}}}}{2}$ will happen and the cluster $s_{k}$ gets eliminated or $a_{\max_{s_{k}}}$ will eliminate $a^{*}$ which are dealt later. So for $\frac{1}{4} \leq \rho_{a} \leq 1$,
\begin{align*}
& \sum_{\substack{i\in A_{s_{k}}^{'}}}\frac{C_{1}(\rho_{a})T^{1-\rho_{a}}}{\Delta_{i}^{'^{{4\rho_{a}-1}}}}\leq \sum_{\substack{i\in A_{s_{k}}^{'}: \\ \Delta_{i}^{'}\geq \Delta_{a_{\max_{s_{k}}}} }}\frac{C_{1}(\rho_{a})T^{1-\rho_{a}}}{\Delta_{a_{\max_{s_{k}}}}^{4\rho_{a}-1}}
   \end{align*}
Summing over all $p-1$ clusters excluding $s^{*}$ the regret is,
\begin{align*}
& \sum_{k=1}^{p-1}\sum_{\substack{i\in A_{s_{k}}^{'}\setminus A_{s^{*}}^{'} : \\ \Delta_{i}^{'}\geq \Delta_{a_{\max_{s_{k}}}} }} \frac{C_{1}(\rho_{a})T^{1-\rho_{a}}}{\Delta_{a_{\max_{s_{k}}}}^{4\rho_{a}-1}} \leq \sum_{i\in A^{'}\setminus A^{'}_{s^{*}}}\frac{C_{1}(\rho_{a})T^{1-\rho_{a}}}{\Delta_{i}^{4\rho_{a}-1}} 
   \end{align*}
...
Following a parallel argument like in Case $a1$, we can prove that for $c_{g_{s_{k}}}=\sqrt{\frac{\rho_{s} \log (\psi T\epsilon_{g_{s_{k}}}^{2})}{2 n_{g_{s_{k}}}}}$ and  $n_{g_{s_{k}}}=\frac{2\log{(\psi T\epsilon_{g_{s_{k}}}^{2})}}{\epsilon_{g_{s_{k}}}}$ we can bound the events,
\begin{align*}
\mathbb{P} \bigg\lbrace \hat{r}_{s_{k}}+c_{g_{s_{k}}} \bigg\rbrace \geq \mathbb{P}\bigg\lbrace \hat{r}^{*}-c_{g_{s_{k}}}\bigg\rbrace
\end{align*}


 in $g_{s_{k}}$-th round by a probability of $\bigg(\dfrac{2}{(\psi  T\epsilon_{g_{s_{k}}}^{2})^{\rho_{s}}}\bigg)$ using Chernoff-Hoeffding bounds and considering independence of events.
%Summing, the two up, the probability that a sub-optimal cluster arm $a_{\max_{s_{k}}}\in C_{g_{s_{k}}}$ is not eliminated
  Now, for any round $g_{s_{k}}$, all the elements of $C_{g_{s_{k}}}$ are the respective maximum payoff arms of their cluster $s_{k}, \forall s_{k}\in S$, and since all the surviving arms are pulled equally in each round and since clusters are fixed so we can bound the maximum probability that a sub-optimal arm $a_{i}\in A^{'}$  and $a_{i}\in s_{k}$ such that $a_{\max_{s_{k}}}\in C_{g_{s_{k}}}$ is not eliminated in the $g_{s_{k}}$-th round by the same probability as above. 
...
In this case for some sub-optimal arm $a_{\max_{s_{k}}}\in C_{g_{s_{k}}}$, we have to bound the probability
	\begin{align*}
...
\begin{align*}
\sum_{i\in A^{'}}\bigg\lbrace \frac{C_{1}(\rho_{a})T^{1-\rho_{a}}}{\Delta_{i}^{4\rho_{a}-1}} + \frac{2C_{1}(\rho_{s})T^{1-\rho_{s}}}{\Delta_{i}^{4\rho_{s}-1}}\bigg\rbrace
\end{align*}
...
%& =
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Case b:} \textit{Either an arm $a_{i}$ is eliminated in round $\max \lbrace m_{i},g_{s_{k}} \rbrace$ or before or else there is no optimal arm $a^{*}\in B_{m_{i}}$} 

\textbf{Case b1:} \textit{$a^{*}\in B_{m_{i}}$ and $a_{i}\in A^{'}$ pulled till $\max \lbrace m_{i},g_{s_{k}} \rbrace$} 
...
\textbf{Case b22:} \textit{$a^{*}\in C_{g}$ and $s^{*}$ gets eliminated by another sub-optimal cluster} 
%\newline
