...
Let $\tilde{A}=\lbrace {i}\in A^{'} | {i}\in s_{k} $ and $ \Delta_{i}> b \rbrace$.
...
From the foregoing, we have to bound the events complementary to that in \eqref{eq:armelim-casea1} for an arm $i$ to not get eliminated. Considering Chernoff-Hoeffding bound and independence of the above events this is done as follows:
...
& + 2\Delta_{i}+\dfrac{64\rho_{s}\log{(\psi T\dfrac{\Delta_{i}^{4}}{16\rho_{s}^{2}})}}{\Delta_{i}} \bigg\rbrace 
 + \sum\limits_{\substack{i\in A: \\ 0\leq\Delta_{i}\leq b}}\bigg(\dfrac{C_{2}(\rho_{s})T^{1-\rho_{s}}}{b^{4\rho_{s} -1}} \bigg)\\
