...
\textbf{Case a:} \textit{Some sub-optimal arm $a_{i}$ is not eliminated in round $\max(m_{i},g_{s_{k}})$ or before and the optimal arm $a^{*}\in C_{\max(m_{i},g_{s_{k}})}$}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
\textbf{Case a2:} \textit{In round $g_{s_{k}}$, $a_{i}\in s_{k}\neq s^{*}, a^{*}\in C_{g_{s_{k}}}$} 
...
\textbf{Case b1:} \textit{$a^{*}\in C_{\max \lbrace m_{i},g_{s_{k}} \rbrace}$ and each $a_{i}\in A^{'}$ is  eliminated on or before $\max \lbrace m_{i},g_{s_{k}} \rbrace$} 
...
%  \hat{r}_{a_{\max_{s_{k}}}} \geq r_{a_{\max_{s_{k}}}} +c_{g_{s_{k}}} \text{ and } \hat{r}_{a_{\max_{s^{*}}}} \leq r_{a_{\max_{s^{*}}}} -c_{g_{s_{k}}}
%\end{align*} 
So $s^{*}$ gets eliminated by $a_{\max_{s_{b}}}$ such that $\sqrt{\rho_{s}\epsilon_{g_{s_{b}}}}\geq\frac{\Delta_{a_{\max_{s_{b}}}}}{2}$ in round $g_{*}$. Also maximal regret per step after eliminating $a^{*}$ is the maximal $\Delta_{j}$ among the remaining arms $a_{j}\in A^{''}$ with $g_{s_{j}}\geq g_{*}$. Following the same way above we can bound the regret as,
...
\textbf{Case b3:} \textit{$a^{*}\notin C_{\max \lbrace m_{i},g_{s_{k}} \rbrace}$ and $s^{*}$ does not get eliminated by another sub-optimal cluster arm} 
